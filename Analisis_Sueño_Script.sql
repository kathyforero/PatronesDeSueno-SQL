...
EXPLAIN SELECT f.temperatura_centigrados, n.nombre AS nivel_ruido, i.nombre AS nivel_iluminacion, ROUND(AVG(s.id_calidad_percibida), 2) AS promedio_calidad
...
-- ¿Cuál es la duración promedio del sueño por usuario?
EXPLAIN SELECT u.nombre, u.apellido, ROUND(AVG(s.duracion_total_minutos), 2) AS duracion_promedio_minutos, ROUND(AVG(s.duracion_total_minutos)/60, 2) AS duracion_promedio_horas
FROM usuarios u
JOIN sesiones_sueno s
ON u.id_usuario = s.id_usuario
GROUP BY u.id_usuario, u.nombre, u.apellido
...
-- ¿Qué porcentaje del sueño corresponde a cada fase (REM, profundo, ligero)?
EXPLAIN SELECT tipo_fase, ROUND(AVG(porcentaje_por_sesion), 2) AS porcentaje_promedio
FROM (
    SELECT f.id_sesion, f.tipo_fase, f.duracion_minutos * 100.0 / SUM(f.duracion_minutos) OVER (PARTITION BY f.id_sesion) AS porcentaje_por_sesion
    FROM fases_sueno f
) t
...
-- Interrupciones y calidad de sueño.
EXPLAIN SELECT s.id_sesion, u.nombre, u.apellido, COUNT(i.id_interrupcion) AS total_interrupciones, ROUND(s.id_calidad_percibida, 2) AS calidad_sueno
FROM sesiones_sueno s
JOIN usuarios u
ON s.id_usuario = u.id_usuario
LEFT JOIN interrupciones i
...
-- Ocupación y calidad de sueño.
EXPLAIN SELECT u.nombre, u.apellido, u.ocupacion, ROUND(AVG(s.id_calidad_percibida), 2) AS promedio_calidad
FROM sesiones_sueno s
JOIN usuarios u ON s.id_usuario = u.id_usuario
GROUP BY u.nombre, u.apellido, u.ocupacion
ORDER BY promedio_calidad ASC;
...
-- Comparar duración total de sueño vs calidad percibida.
EXPLAIN SELECT RANK() OVER(ORDER BY ROUND(s.duracion_total_minutos/60, 2) DESC) AS Ranking, 
		s.id_sesion, u.nombre, u.apellido, ROUND(s.duracion_total_minutos/60, 2) AS duracion_total_horas, c.nombre AS calidad
FROM sesiones_sueno s
JOIN usuarios u
ON s.id_usuario = u.id_usuario
...
-- Relación peso/altura (IMC aproximado) vs calidad del sueño.
EXPLAIN SELECT u.nombre, u.apellido, ROUND(u.peso_kg / ((u.altura_cm/100)*(u.altura_cm/100)), 1) AS imc, ROUND(AVG(s.id_calidad_percibida), 2) AS calidad_promedio
FROM usuarios u
JOIN sesiones_sueno s
ON u.id_usuario = s.id_usuario
GROUP BY u.id_usuario, u.nombre, u.apellido
...
-- Lugar de descanso y calidad de sueño.
EXPLAIN WITH promedio_calidad_lugar AS (
	SELECT lugar, ROUND(AVG(id_calidad_percibida), 2) AS promedio_calidad
    FROM sesiones_sueno
    GROUP BY lugar
)
...
CREATE INDEX idx_sesiones_lugar ON sesiones_sueno(lugar); -- Necesario

-- Identificar por usuario
EXPLAIN SELECT u.nombre, u.apellido, c.nombre AS calidad, f.temperatura_centigrados, f.id_nivel_iluminacion, f.id_nivel_ruido
FROM usuarios u
JOIN sesiones_sueno s
ON u.id_usuario = s.id_usuario
JOIN calidad_percibida c
